			...
			<div class="fav-box">
				...
				<c:forEach var="fav" items="${favList}">
					<div class="fav-item">
						...
						<c:out value="${fav.name}" default="(이름 없음)" />
					</div>
				</c:forEach>
				<c:if test="${empty favList}">
					<div style="color: #bbb;">찜한 장소가 없습니다.</div>
				</c:if>
			</div>
			<!-- 리뷰 목록 -->
				...
				<div class="reviews-list">
					<c:forEach var="review" items="${reviewList}">
						<div class="review-item">
							<span class="flag">🚩</span> <b><c:out
									value="${review.name}" default="(장소 없음)" /></b> <span
					...
					<c:if test="${empty reviewList}">
						<div style="color: #bbb;">작성한 리뷰가 없습니다.</div>
					</c:if>
				</div>
			</div>
		</div>
